...
namespace mcmc_utilities
{
  ...
  public:
    ...
    {
      return std::shared_ptr<node<T,T_vector> >(new abs_node<T,T_vector>);
    }

    std::shared_ptr<vnode<T,T_vector> > clone()const override
    {
      return std::shared_ptr<vnode<T,T_vector> >(new abs_vnode<T,T_vector>(*this));
    }
  };
  ...
  template <typename T,template <typename TE> class T_vector>
  abs_vnode<T,T_vector> vabs(const vnode<T,T_vector>& n1)
  {
    auto result= abs_vnode<T,T_vector>(std::string("abs")+node_count<abs_vnode<T,T_vector> >(),{n1,(size_t)0});
    result.named=false;
    return result;
  }
