...
namespace mcmc_utilities
{
  ...
  public:
    ...
    {
      return std::shared_ptr<node<T_p,T_var1> >(new logit_node<T_p,T_var1>);
    }

    std::shared_ptr<vnode<T_p,T_var1> > clone()const override
    {
      return std::shared_ptr<vnode<T_p,T_var1> >(new logit_vnode<T_p,T_var1>(*this));
    }
  };
  ...
  template <typename T_p,typename T_var1>
  logit_vnode<T_p,T_var1> vlogit(const vnode<T_p,T_var1>& n1)
  {
    auto result= logit_vnode<T_p,T_var1>(std::string("logit")+node_count<logit_vnode<T_p,T_var1> >(),{n1,(size_t)0});
    result.named=false;
    return result;
  }
